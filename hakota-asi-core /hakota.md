...
# Hakota AI Core Framework (`Hakota-asi-core`)
...
This document outlines the vision and architecture of the `Hakota-asi-core` project, which serves as the core, model-agnostic AI framework.
...
> - **Logic Push-Down**: Maximize business logic and feature orchestration in `Hakota-asi-core` (like `AbstractAsiContainer` or `Agi` lifecycle methods). The more logic we keep in the core, the more we can reuse it across different UI layers (Swing, JavaFX, CLI, Web).
...
This project contains the foundational logic, while provider-specific implementations are developed in separate "adapter" projects (e.g., `Hakota-asi-gemini`).

---

## 2. V2 Core Architecture Summary

### 2.1. Core Orchestration (`uno.Hakota.asi.agi`)
...
### 2.2. Model-Agnostic Domain (`uno.Hakota.asi.model.*`)
...
### 2.4. V2 Tool Framework (`uno.Hakota.asi.tool.*`)
...
### 2.5. Resource & State Management (`uno.Hakota.asi.resource.*`)
